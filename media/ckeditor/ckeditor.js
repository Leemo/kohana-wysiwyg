...
for(q in h)"#"in B[q]||delete h[q];for(q in h)j.elements[q]=i(t,b.config.fillEmptyBlocks!==false);j.root=i(t);
...
"tbody"],D=CKEDITOR.tools.extend({},B.$blockLimit,B.$block),u={
  ...
  elements:{$:function(a){var b=a.attributes;if(b){if(b["data-cke-temp"])return false;for(var c=["name","href","src"],d,n=0;n<c.length;n++){d="data-cke-saved-"+c[n];d in b&&delete b[c[n]]}}return a},
  ...
  if(a.collapsed){
    ...
    if(!g){
      ...
      for(var g=y(this,c),q=g.hasAttributes(),s=n.getCommonAncestor(),p={},r={},u={},z={},A,B,D;g&&s;){
        if(s.getName()==d){
          for(A in e.attributes)if(!z[A]&&(D=s.getAttribute(B)))g.getAttribute(A)==D?r[A]=1:z[A]=1;for(B in e.styles)if(!u[B]&&(D=s.getStyle(B)))g.getStyle(B)==D?p[B]=1:u[B]=1
            }
            s=s.getParent()
        }
...
function d(a){
  ...
  for(var d;d=c.getNextParagraph();)if(this.checkElementRemovable(d))if(d.is("pre")){
    ...
    k(d,e)
    }else n.call(this,d);a.moveToBookmark(b)
  }
  function k(a,b){
  var c=!b;
  if(c){
  ...
  c.contextMenu.addListener(function(a){
    if(a&&!a.isReadOnly()){
      ...
      if(b=="input")switch(a.getAttribute("type")){
        ...
        if(b==
      "img"&&a.data("cke-real-element-type")=="hiddenfield")return{
...
c.on("doubleclick",function(a){
  ...
  if(!c.blockless&&b.is("form"))a.data.dialog="form";
    ...
    case "button":case "submit":case "reset":a.data.dialog=
      "button";
      break;
    ...
    case "image":a.data.dialog="imagebutton";
      break;
    default:a.data.dialog="textfield"
      }
    })
},
  ...
  function J(a,b,d){
    ...
    o(d,{
      ...
      place:function(){
        ...
        for(var O in g)g[O]=CKEDITOR.tools.cssLength(g[O]);this.setStyles(g)
        },
      look:function(a){
        if(this.oldLook!=a){
...
(function(){
  function h(a,b){
    ...
    a.on("paste",function(f){
      ...
      if(c&&(b||/(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/.test(c))){var g=h(e,function(){if(g)a.fire("paste",
        d);
...
filter:function(a,h){
  for(var e=a._.removeFormat.filters,f=0;f<e.length;f++)if(!1===e[f](h))return!1;return!0
  }
};
